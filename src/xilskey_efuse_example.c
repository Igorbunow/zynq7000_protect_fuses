...
int main()
{
	...
	if (PsStatusBits & (1 << 7)) {
		xil_printf("EfusePS status bit  7: ARM Core 2 blocked\n\r");
	}
	else {
		xil_printf("EfusePS status bit  7: ARM Core 2 enabled\n\r");
	}

	if (PsStatusBits & XSK_EFUSEPS_STATUS_DFT_MODE_DISABLE) {
		xil_printf("EfusePS status bit  8: DFT mode is disabled\n\r");
	}
	else {
		xil_printf("EfusePS status bit  8: DFT mode is enabled\n\r");
	}

	if (PsStatusBits & XSK_EFUSEPS_STATUS_DFT_JTAG_DISABLE) {
		xil_printf("EfusePS status bit  9: DFT JTAG is disabled\n\r");
	}
	else {
		xil_printf("EfusePS status bit  9: DFT JTAG is enabled\n\r");
	}

    if(PsStatusBits & XSK_EFUSEPS_STATUS_RSA_EN) {
		xil_printf("EfusePS status bit 10: RSA authentication of "
			"fsbl enabled\n\r");
    }else {
		xil_printf("EfusePS status bit 10: RSA authentication of "
			"fsbl disabled\n\r");
    ...
    if(PsStatusBits & XSK_EFUSEPS_STATUS_ROM_128_CRC) {
		xil_printf("EfusePS status bit 11: 128k CRC check on ROM enabled\n\r");
    }else {
		xil_printf("EfusePS status bit 11: 128k CRC check on ROM disabled\n\r");
    }

    if((PsStatusBits & XSK_EFUSEPS_STATUS_WP_BIT_LOW) ||
		(PsStatusBits & XSK_EFUSEPS_STATUS_WP_BIT_HIGH)) {
	xil_printf("EfusePS status bit 12|13: Write protect enabled\n\r");
    }else {
	xil_printf("EfusePS status bit 12|13: Write protect disabled\n\r");
    }

	if (PsStatusBits & (1 << 15)) {
		xil_printf("EfusePS status bit 15: BAD\n\r");
	}
	else {
		xil_printf("EfusePS status bit 15: OK\n\r");
	}
    ...
    if (PsStatus != XST_SUCCESS){
	printf("PS EFUSE RSA Key Hash reading failed\n");
	//goto EFUSE_ERROR;
    }
    else{
		/**
